...
namespace QuanLyBanHang.Class
{
    internal class Function
    {
        ...
        {
            Con = new SqlConnection(@"Data Source=DESKTOP-E8GTQJS\SQLEXPRESS;Initial Catalog=QuanLyBH; Integrated Security=True");
            Con.Open();
        }
        ...
        public static string CreateKey(string tiento)
        {
            ...
            /*if (partsTime[2].Substring(0, 2) == "PM")
                partsTime[0] = ConvertTimeTo24(partsTime[0]);
            if (partsTime[2].Substring(0, 2) == "AM")
                if (partsTime[0].Length == 1)
                    partsTime[0] = "0" + partsTime[0];
            MessageBox.Show(partsTime[2]);*/
            //Xóa ký tự trắng và PM hoặc AM
            // partsTime[2] = partsTime[2].Remove(2,1);
            string t;
            ...
            try
            {
                using (SqlCommand cmd = new SqlCommand(sql, Con))
                {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            // Kiểm tra xem giá trị có rỗng hay không trước khi chuyển đổi
                            ...
                            {
                                // Xử lý trường hợp giá trị rỗng nếu cần
                                ma = ""; // hoặc có thể đặt giá trị mặc định khác tùy thuộc vào yêu cầu của bạn
                            }
                        }
                    }
